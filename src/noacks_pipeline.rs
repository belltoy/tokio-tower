//! Pipelined client with optional acks.
//!
use crate::mediator;
use crate::wrappers::*;
use crate::Error;
...
/// A request which must answer if it require acks.
pub trait OptionalAcks {

    /// Return whether this request requires acks.
    fn require_acks(&self) -> bool;
}
...
where
    ...
    Request: 'static + Send + OptionalAcks,
    NT::Item: 'static + Send,
    ...
    T::Ok: 'static + Send,
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
            ...
            while let Poll::Ready(r) = transport.as_mut().poll_ready(cx) {
                ...
                match this.mediator.try_recv(cx) {
                    Poll::Ready(Some(ClientRequest {
                        ...
                        let require_acks = req.require_acks();
        ...
        while this.in_flight.load(atomic::Ordering::Acquire) != 0 {
            ...
            if let Pending {require_acks: false, ref span, ..} = this.responses.front().ok_or(Error::Desynchronized)? {
                // we won't waiting for acks, so we can't poll the stream
                tracing::trace!(parent: span, "no acks");
                let _ = this.responses.pop_front();
    ...
    fn call(&mut self, req: Request) -> Self::Future {
        ...
        Box::pin(async move {
            match r {
                Ok(()) => {
                    if !require_acks {
                        tracing::trace!("no acks response returned");
                        Ok(None)
                    } else {
                        match rx.await {
                            Ok(r) => {
